...
function Mock(logger, prismUtils, mockFilenameGenerator, responseDelay) {
  ...
  function delayResponse(path, prism, req, res) {
    ...
    setTimeout(function() {
      writeHttpResponse(path, res, prism);
  ...
  function writeHttpResponse(path, res, prism) {
    ...
    if(prism.config.recordHeaders && prism.config.recordHeaders.length) {
      for (var i = 0; i < prism.config.recordHeaders.length; i++) {
        var headerName = prism.config.recordHeaders[i]
        var headerNameLower = headerName.toLowerCase();

        if (headerNameLower in response.headers) {
          actualResponse[headerName] = response.headers[headerNameLower];
        }
      }
    }
